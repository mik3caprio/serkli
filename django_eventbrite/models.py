...
class Event(models.Model):
    ...
    def eventbrite_fees(self):
        eb_fees = sum(map(lambda a: a.fee, self.attendees.filter(refunded=False)), Money(0, DEFAULT_CURRENCY))
        return eb_fees
    eventbrite_fees.short_description='EB fees'

    def ticket_sales(self):
        gross = sum(map(lambda a: a.gross, self.attendees.filter(refunded=False)), Money(0, DEFAULT_CURRENCY))
        eb_fees = sum(map(lambda a: a.fee, self.attendees.filter(refunded=False)), Money(0, DEFAULT_CURRENCY))
